...
BubbleBullet::BubbleBullet()
{
	...
	set_collision_size({ 85,85 });			//设置碰撞箱大小
	...
	//初始化爆炸动画
	Atlas* atlas_bubble_explode = ResourcesManger::GetInstance().find_atlas("atlas_bubbles");
...
void BubbleBullet::begin(Character* character)
{
...
void BubbleBullet::play_sound()
{

}


void BubbleBullet::on_update(float delta)
{
	if (!is_anble)
		return;
	...
	else
	{
		...
		set_collision_size({ size.x += speed * delta,size.y += speed * delta });			//设置碰撞箱大小
	}
}
